/*
Name: Alexander DeVictoria
Email: adevi002@ucr.edu
gdoc: https://docs.google.com/document/d/1ZpIZN5jGF_ziYU9JE2SHWjYoGj-00dsKM1bqWr1HuSM/edit?usp=sharing
YouTube video: https://www.youtube.com/watch?v=_3mecDYsbIA
*/
...
//changed Ball weight slider so it can be stepped by 0.001 instead of 0.01
//made the ball level out faster
//added plot points so the user can see the exact distances and the exact times
//added a disable I and a disable D button so the user can remove the I and D variables
...
var plotsP = new Array(maxNumPoints);
var plotsPi = new Array(maxNumPoints);
var yMin;
var yMax;
var xMin;
var xMax;
...
function updateController()
{
    ...
    //Actuator += baseSpeed;                                        // makes graph more accurate
...
function startSimulation()
{
    if(validateForm())
    {
        ...
        maxSpeed = (p*Desired+i*integMax) > actuatorMax ? actuatorMax : (p*Desired+i*integMax) + 1;  //makes fan spin
        pidAnim.init(Desired);      //animates ball and fan
        timer = setInterval(simulate,timerPeriod);

        plot = $.jqplot("chartdiv",[plots],{            //plots graph
...
function updateSystem()
{
    ...
    if((timeCnt%sampleRate) == 0)
    {
        ...
        //plotsP[indexToWrite][0] = Error;
        //plotsP[indexToWrite][1] = Actual;
        //plotsPi[indexToWrite][0] = dt*timeCnt;
        //plotsPi[indexToWrite][1] = Actual;
...
function simulate()
{
    ...
    if((timeCnt%sampleRate) == 0)
    {
        ...
        //plot.series[1].data = plotsP;
        //plot.series[2].data = plotsPi;
...
function disableI() {                               //disables the use of I variable
    if($("#i").attr("disabled", true)) {
        $("#i").removeAttr("disabled");
        $("#i").hide();
        $("#I_label").removeAttr("disabled");
        $("#I_label").hide();
        i = 0;
    }
}

function disableD() {                               //disables the use of D variable
    if($("#d").attr("disabled", true)) {
        $("#d").removeAttr("disabled");
        $("#d").hide();
        $("#D_label").removeAttr("disabled");
        $("#D_label").hide();
        d = 0;
    }
}
